  ...
  return (
    <>
      ...
      <main>
        ...
        {/* Products grid */}
        <section className="shop" id="shop" style={{ paddingTop: '5rem', paddingBottom: '5rem' }}>
          <div className="shop__header">
            <div className="section-header">
              <div>
                <p className="eyebrow eyebrow--light">All Terrariums</p>
                <h2>
                  {products.length > 0
                    ? <>{products.length} <em>terrariums</em></>
            ...
            {products.length > 0 ? products.map((p) => (
              <Link key={p.id} href={`/products/${p.slug}`} className="product-card card--visible">
                <div className="product-card__img" style={{ background: 'linear-gradient(150deg,#dce8d8,#c8d8c4)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  {p.image_url && (
                    <Image src={p.image_url} alt={p.name} fill style={{ objectFit: 'cover' }} sizes="300px" />
                  )}
                </div>
